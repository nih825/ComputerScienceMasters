...
int returnLocation(int i, int j, int k, int nx, int ny, int nz){
    return (i-1)*ny*nz+(j-1)*nz+k-1+6; //subtracts 1 to convert to typical array based notation and adds 6 as the first 6 rows are allocated for the boundary conditions
}
...
vector<vector<double>> createA(int nx, int ny, int nz, double alpha, double dt, double dx, double dy, double dz,bool wraparound,double boundary1, double boundary2, double boundary3, double boundary4, double boundary5, double boundary6){
    ...
    //Create first 6 rows for boundary conditions
    a[0][0]=1;
    a[1][1]=1;
    a[2][2]=1;
    a[3][3]=1;
        ...
        for(int i=1; i<nx+1; i++){
            for(int j=1; j<ny+1; j++){
                for(int z=1; z<nz+1; z++) {
                    ...
                    //figure out if current cell neighbors boundary and handle appropriately 
                    if(i+1==nx+1)
                        a[location][0]=-c1;
                    else
        ...
        for(int i=1; i<nx+1; i++){
            for(int j=1; j<ny+1; j++){
                for(int z=1; z<nz+1; z++) {
                    int location=returnLocation(i,j,z,nx,ny,nz);
                    a[location][location]=1+2*c1+2*c2+2*c3;
                    //same as above only in this case handles boundary condition by looking at term for neighbor on opposite side rather than at the boundary
                    int xlocright=returnLocation(i+1,j,z,nx,ny,nz);
                    if(i+1==nx+1)
                    ...
                    //Now fill in entries in the matrix
                    a[location][xlocleft]=-c1;
                    a[location][xlocright]=-c1;
                    a[location][ylocleft]=-c2;
                    a[location][ylocright]=-c2;
