...
void mglin(double **u, int n, int ncycle, double c, double sourceMult, double **s){
/*
...
*/
  ...
  slvsml(iu[1],irho[1], c, sourceMult, s);          /* solve the small system directly */
  ...
  for (j=2;j<=ngrid;j++) {        /* loop over coarse to fine, starting at level 2 */
    ...
    */
	...
	for (jpre=1;jpre<=NPRE;jpre++)  /* NPRE g-s sweeps on the finest (relatively) scale */
	  relax(iu[jj],irhs[jj],nf, c, sourceMult, s);
	resid(ires[jj],iu[jj],irhs[jj],nf,c, sourceMult, s); /* compute res on finest scale, store in ires */
	...
	fill0(iu[jj-1],nf);              /* set the initial solution guess to zero */
      ...
      slvsml(iu[1],irhs[1], c, sourceMult, s);                  /* solve the small problem exactly */
