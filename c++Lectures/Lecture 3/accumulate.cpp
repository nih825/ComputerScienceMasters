...
vector<double> ex2_1(vector<double> v)
{
...
void ex2_1_lambda(vector<double> v)
{
	vector<double> result;
	result.resize(v.size());
	transform(v.begin(), v.end(), result.begin(), [](double d) { return d*d; });
	cout << "result = (";
	ostream_iterator<double> os(cout, ", ");
	copy(result.begin(), result.end(), os);
	cout << ")" << endl;
...
double square_and_add_to_accumulator(double old_accumulator, double next) {
...
{
	double values[] = {1, 2, 3};
	vector<double> v(values, values + sizeof(values)/sizeof(double));
// C++11: vector<double> v = {1, 2, 3};
	ex2_1(v);
	ex2_1_alternate(v);
	ex2_1_lambda(v);
	ex2_2(v);
